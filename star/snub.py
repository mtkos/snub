...
from numpy.linalg import det, matrix_power, inv
...
snub_color = eval(lines[3 + 4*n_axes])

ntriangles = 0
for x in mult:
    ntriangles += x*len(group)

ntriangles += len(group) # snub triangles
...
triangle = [vertex, inv(R[1])@vertex, R[0]@vertex]
for r in group:
    t = [r@v for v in triangle]
    if det(r) < 0:
        t.reverse()
    write_triangle(t, snub_color, f)
#    t.reverse()
#    write_triangle(t, snub_color, f)
